...
class Instrument {
    ...
    * */
     ...
     */
    initAudioContext() {
        const Context = win.AudioContext || win.webkitAudioContext, hasOldContext = !!Instrument.audioContext;
        if (Context) {
     ...
     */
    getValidFrequency(frequency, min, max) {
        const validFrequencies = this.options.allowedFrequencies, maximum = pick(max, Infinity), minimum = pick(min, -Infinity);
     ...
     */
    setFrequency(frequency, frequencyLimits) {
        const limits = frequencyLimits || {}, validFrequency = this.getValidFrequency(frequency, limits.min, limits.max);
        if (this.options.type === 'oscillator') {
     ...
     */
    play(options) {
        const instrument = this, duration = options.duration || 0, 
        ...
        setOrStartTimer = function (value, setter, setterData) {
            const target = options.duration, callbackInterval = instrument.options.playCallbackInterval;
            let currentDurationIx = 0;
            if (typeof value === 'function') {
                const timer = setInterval(function () {
                    currentDurationIx++;
                    const curTime = (currentDurationIx *
    ...
    stop(immediately, onStopped, callbackData) {
        const instr = this, reset = function () {
