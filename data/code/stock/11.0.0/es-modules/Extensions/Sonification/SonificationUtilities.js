...
const SonificationUtilities = {
    ...
    calculateDataExtremes: function (chart, prop) {
        return chart.series.reduce(function (extremes, series) {
            ...
            series.points.forEach(function (point) {
                const val = typeof point[prop] !== 'undefined' ?
    ...
    virtualAxisTranslate: function (value, dataExtremes, limits, invert) {
        const lenValueAxis = dataExtremes.max - dataExtremes.min, lenVirtualAxis = Math.abs(limits.max - limits.min), valueDelta = invert ?
...
function getExtremesForInstrumentProps(chart, instruments, dataExtremes) {
    const defaultInstrumentDef = (chart.options.sonification &&
        chart.options.sonification.defaultInstrumentOptions), optionDefToInstrDef = (optionDef) => ({
    ...
    chart.series.forEach((series) => {
        const instrOptions = (series.options.sonification &&
    ...
    return (allInstrumentDefinitions).reduce(function (newExtremes, instrumentDefinition) {
        Object.keys(instrumentDefinition.instrumentMapping || {}).forEach(function (instrumentParameter) {
            const value = instrumentDefinition.instrumentMapping[instrumentParameter];
