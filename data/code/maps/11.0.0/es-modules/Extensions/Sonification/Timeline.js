...
class Timeline {
     ...
     */
    playPaths(direction) {
        const timeline = this, signalHandler = timeline.signalHandler;
        ...
        const curPaths = splat(this.paths[this.cursor]), nextPaths = this.paths[this.cursor + direction], 
        // Play a path
        playPath = function (path) {
            ...
            path[direction > 0 ? 'play' : 'rewind'](function (callbackData) {
                ...
                const cancelled = callbackData && callbackData.cancelled, signalData = {
                    path: path,
                    cancelled: cancelled
                };
